...
*    ENDLOOP.
    LOOP AT lt_rows INTO DATA(ls_rows).
      ls_excel_data-entrysheet = lv_entrysheet = |{ ls_rows-entrysheet ALPHA = IN }|.
      ls_excel_data-ebeln      = |{ ls_rows-ebeln ALPHA = IN }|.
      ls_excel_data-ebelp      = |{ ls_rows-ebelp ALPHA = IN }|.
      ls_excel_data-quantity = CONV #( ls_rows-quantity ).

      APPEND ls_excel_data TO lt_excel_data.

      CLEAR: ls_rows, ls_excel_data.
    ENDLOOP.
